...
# SCRIPT TO BUILD THE MONTHLY ENROLLMENT LIST DATA  CSV FILE 
...
# 3 READ THE DOWNLOADED DATA EXCEL FILES ------------------------------------
# FROM CARE IMPACT SYSTEM, MEMBER DASHBOARD
...
# * Alternative Method - Apply change to vectortypes ----
# vectortypes <- read_excel(filenames_list[1]) %>% 
#   summarise_all(class) %>% slice(1) %>% unlist(., use.names=FALSE)
# vectortypes # WITH logical fields
...
fx_readfiles <- function(filename){
  ...
  readxl::read_excel(filename, col_types = vectortypes)}
